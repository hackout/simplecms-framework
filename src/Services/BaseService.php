...
class BaseService
{
    ...
    {
        $className = $this->className !== '' ? $this->className : ($this->model instanceof Model ? $this->model::class : '');

        if ($className === '') {
            throw new \RuntimeException('No model class has been configured for this service.');
        }

        return new $className;
    }

    private function autoModel(): void
    {
        $model = $this->getModel();
        ...
        $this->orderKey = $model->timestamps ? $this->autoOrderKey() : $this->primaryKey;
    ...
    public function setModel(string $className)
    {
        if (! class_exists($className)) {
            throw new \InvalidArgumentException(sprintf('Model class "%s" does not exist.', $className));
        }
    ...
    public function getItem(string $className = null): mixed
    {
        ...
        if ($className !== '' && $this->item instanceof $className) {
            return $this->item;
        }
